...
* **Karaoke-style lyrics**: the line being sung is highlighted and scrolls into view (inline under the player or full screen); tap a line to jump there. Real `.lrc` timestamps drive the highlight; lyrics *without* timestamps (e.g. plain text stored in the file's tags) follow the playback position approximately and are labelled as such. Sources, in order: a file you imported for the song → the same-name `.lrc` (tolerant of case, ي/ی, numbering; UTF-8/UTF-16/Windows-1256; offsets) → lyrics **embedded in the file's tags** (MP3 ID3 `USLT`, FLAC `LYRICS`). Finding sidecar files needs **no broad permission**: pick your music folder once with the system folder picker (Android 10+); the app indexes the `.lrc` files in it and caches the index locally. A built-in *“why wasn't it found?”* diagnosis explains what was checked.
